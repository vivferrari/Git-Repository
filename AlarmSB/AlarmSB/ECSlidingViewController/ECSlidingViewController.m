...
- (void)resetTopView
{
    stateVc = [[StateViewController alloc]init];
    ...
    if([self underLeftShowing]){
        ++stateCounter;
        if(stateCounter%2==0){
            if((stateCounter/2)%2==0){
                labelString = @"OFF";
                stateVc.labelText = labelString;
                NSLog(@"OFF %@",stateVc.labelText);
                vc.alarmState = NO;
            }
            else{
                labelString = @"ON";
                stateVc.labelText = labelString;
                NSLog(@"ON %@",stateVc.labelText);
                vc.alarmState = YES;
                [vc setAlarmTime];
            }
